...
colors="${colors}\""
cat ~/.conky/conky-pywal/templates/ram_template2.lua | awk -v srch="COLORFIELD" -v repl="$colors" '{ sub(srch,repl,$0); print $0 }' > ~/.conky/conky-pywal/lua/ram.lua
cat ~/.conky/conky-pywal/templates/cpu_template2.lua | awk -v srch="COLORFIELD" -v repl="$colors" '{ sub(srch,repl,$0); print $0 }' > ~/.conky/conky-pywal/lua/cpu.lua
cat ~/.conky/conky-pywal/templates/ssd_template2.lua | awk -v srch="COLORFIELD" -v repl="$colors" '{ sub(srch,repl,$0); print $0 }' > ~/.conky/conky-pywal/lua/ssd.lua
