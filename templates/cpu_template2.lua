...
function draw_cpu(cr, w, h)
    ...
    --Used CPU ring
    cpu_usage = (tonumber(conky_parse("${cpu cpu0}")) +
				 ...
				 tonumber(conky_parse("${cpu cpu23}")))/24
    cpu_used = math.floor(360*cpu_usage/100)
    ...
    --temp = tonumber(conky_parse("${acpitemp}"))
    ...
    -- CPU Text
    cpu_usage_string = string.format("CPU Usage: %.2f", cpu_usage)
    ct = cairo_text_extents_t:create()
    cairo_text_extents(cr,cpu_usage_string,ct)
    cairo_move_to(cr,w/2-ct.width/2,h/2+ct.height/2 +45)
    cairo_show_text(cr,cpu_usage_string)
    
    -- Temp Text
    --temp_string = string.format("Temp: %.2f", temp)
    --ct = cairo_text_extents_t:create()
    --cairo_text_extents(cr,temp_string,ct)
    --cairo_move_to(cr,w/2-ct.width/2,h/2+ct.height/2 +50)
    --cairo_show_text(cr,temp_string)
