...
function draw_ram(cr, w, h)
    ...
    -- RAM usage
    used_ram = conky_parse("${mem}")
    size_ram = conky_parse("${memmax}")
    ct = cairo_text_extents_t:create()
    cairo_text_extents(cr,"RAM "..used_ram.."/"..size_ram,ct)
    cairo_move_to(cr,w/2-ct.width/2,h/2+ct.height/2 +35)
    cairo_show_text(cr,"RAM "..used_ram.."/"..size_ram)
